...
Status: **executed 2026-07-08** — all phases shipped the same day (Phase 0: v0.10.0–0.12.0; Phase 1: v0.13.0; Phase 2: v0.14.0–0.16.0; Phase 4: v0.17.0–0.17.1; Phase 3: v0.18.0, user-approved invariant rewrite), and the `~/.claude` allowlist repo was initialized on this machine. The "does not decide" list stays live; the first reflect pass over real multi-class runs should validate the autonomy behavior (see Phase 3 note). Created 2026-07-08 from a user-directed reframing: the suite's mission grows from "tidy CLAUDE.md" to **maintaining every prose instruction file Claude Code loads** — keeping them slim and tidy, removing unnecessary content, relocating misplaced content, and compacting/optimizing wording. Human-in-the-loop only for decisions **only the user can make**; everything else autonomous, using iterative refinement loops.
...
- **Phase 0 — land the scope-tensions plan** (items 2–5; item 1 rides along). The placement taxonomy (correctly-scoped, rule-vs-memory, import-following, rules destination) is the relocation engine every later phase reuses. MINOR bumps, already specced there. **Done 2026-07-08: v0.10.0 (items 2+3), v0.11.0 (item 4), v0.12.0 (items 5+1).**
- **Phase 1 — `~/.claude` versioning bootstrap.** New preflight capability: detect, propose, create the allowlist repo on approval (option 1 above). Prerequisite for putting user-level targets in the autonomous tier. MINOR. **Done 2026-07-08: v0.13.0** (the capability; the actual init on this machine happens on first use or on request).
- **Phase 2 — generalize targets.** Step 2's target-finding becomes a target-class registry (each class: locations, load behavior, size guidance, class-specific tests). First new classes: user-level CLAUDE.md/rules (they reuse the existing tests nearly verbatim), then skills (500-line guidance, description-quality test, progressive-disclosure test: "should this SKILL.md section be a lazily-read supporting file?"), then memory (index-is-an-index test, topic-file dedup, stale-fact pruning, promotion flags — plus the snapshot mechanism). Each class lands as its own MINOR release so run records accumulate per class. **Done 2026-07-08: v0.14.0 (registry + user-level), v0.15.0 (skills), v0.16.0 (memory).**
- **Phase 3 — autonomy rework.** Invariant 1 rewrite (MAJOR, final wording confirmed with the user), the reversibility gate, the iterative self-review loop, lean CHALLENGE. Deliberately *after* Phase 2 has produced real multi-class run records — the autonomy change is the riskiest step and should be informed by evidence of where the skill's judgment was actually right and wrong. **Done 2026-07-08: v0.18.0** — executed ahead of real multi-class run records at the user's explicit direction (invariant wording approved verbatim; shipped MAJOR-noted at 0.18.0 per pre-1.0 precedent, user decision). The evidence-informed review this phase was meant to wait for now happens *after* the fact: the first reflect pass over real multi-class runs should treat the autonomy behavior as a standing hypothesis to validate, and any counter-evidence as top-priority.
- **Phase 4 — reflect-skill alignment.** Run records gain a target-class field; the reflect skill's signals and the pruning loop work per class; README/invariants updated to describe the mission. PATCH/MINOR. **Done 2026-07-08: v0.17.0** (executed before Phase 3, which requires the user's invariant-wording sign-off; invariants themselves change only in Phase 3).
...
- **2026-07-08 — executed.** All phases implemented across nine releases (0.10.0 → 0.18.0), each with an adversarial-review pass before commit (pass findings shipped as 0.17.1 and as inline fixes). User decisions during execution: invariant-1 wording approved verbatim; Phase 3 shipped MAJOR-noted at 0.18.0 (pre-1.0 precedent); `~/.claude` allowlist repo initialized immediately (7 files tracked, verified nothing sensitive staged). Deliberate deviation, user-directed: Phase 3 ran before real multi-class run records existed — logged in the Phase 3 note as a hypothesis for the first reflect pass to validate.
