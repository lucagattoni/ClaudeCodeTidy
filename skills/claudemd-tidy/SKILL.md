...
4. Inventory the possible relocation homes: `.claude/commands/` and `.claude/skills/` (per-task procedures), `.claude/rules/<topic>.md` (facts/constraints scoped to part of the codebase, as a path-scoped rule — follow the repo's existing convention: raise a question before introducing `.claude/rules/` to a repo that doesn't use it), `docs/` or `plans/` (background/design), `README.md` (human-facing description).
