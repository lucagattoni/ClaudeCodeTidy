...
- **Preflight.** Verifies it's in a git repo, syncs (`git fetch`, branch/dirty/ahead-behind check), and checks repo visibility — public or unknown visibility activates the global PRIMARY CHECK for everything the skill writes.
- **Repo picture first.** Before analyzing a single line, the skill builds a complete picture of the repo — file tree, README/docs, every skill and hook, settings, manifests, CI, recent git history — and must pass a comprehension gate (what the repo does, how it runs, which agents operate on it, which conventions demonstrably hold). Every verdict is grounded in what the repo *is*, not in what the CLAUDE.md claims.
