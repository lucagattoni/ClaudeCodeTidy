...
**Version 0.19.2** ([changelog](CHANGELOG.md)) · Two personal Claude Code skills that maintain your whole Claude Code instruction surface — project and user-level CLAUDE.md, `.claude/rules/`, SKILL.md files, and auto memory — keeping every file slim without losing information, and improving themselves from the experience of real runs.
...
- **Scope.** Target classes: project CLAUDE.md files (default), user-level files (`--user`: your global `~/.claude/CLAUDE.md` and `~/.claude/rules/`), skills (`--skills`: SKILL.md description quality, progressive disclosure, frontmatter sanity — never this suite's own two skills, which only the reflect loop may change), and auto memory (`--memory`: index kept an index, topic dedup, stale-fact pruning, promotion flags — always snapshot-first to `${CLAUDE_PLUGIN_DATA}/memory-backups/`, and memory content never lands in a repo-tracked file) — same phases, class-specific tests; `--all` runs every class. By default it sweeps every CLAUDE.md in the repo, root and nested — including gitignored personal-override files (e.g. `CLAUDE.local.md`-style conventions), which a plain `git ls-files`/untracked-file check would otherwise miss entirely. The audit covers the **effective** CLAUDE.md: live `@imports` are detected (code-span mentions excluded) and followed up to the documented four-hop limit, so imported content — an `@AGENTS.md`, a `@docs/...` file — is interrogated like inline content. External (out-of-repo) imports are hedged as possibly inert, since a declined approval dialog silently disables them. An unimported `AGENTS.md` is flagged: Claude Code never reads it, in any session (project-class targets only — user-level, skills, and memory have no AGENTS.md-equivalent surface). Edits follow a strict policy: in-repo instruction files are edited in place, dual-purpose files (README, manifests) never — findings land on the import line — and out-of-repo files only via individually-confirmed CHALLENGEs.
...
- **Reversibility gate.** The full plan (CHALLENGE questions **grouped by stakes** — destructive/safety-relevant first, then widely-depended-on rules, then everything else batched as minor CHALLENGEs — then the verdict table, destinations, evidence, before/after line estimates) is always presented. Git-tracked (or snapshot-protected) targets are then edited **autonomously**, one commit per iteration, with the autonomy announced; unversioned, unprotected targets wait for confirmation, with partial approval and amended verdicts supported. CHALLENGEs, out-of-repo writes, and PRIMARY-CHECK flags always stop for the user — CHALLENGE itself is lean by principle: reserved for questions only the user can answer, never for anything evidence could settle.
- **Apply discipline.** Re-checks the target's own branching/worktree conventions immediately before the first commit — even for a trivial single-file edit, and even when the file being committed is itself the source of those conventions — before falling back to a size-based default (multi-file → branch, single-file compress-only → may go direct); relocates first (with two-way cross-links), then compresses, then deletes; greps for every moved term and updates all referencing docs in the same change; updates the repo's CHANGELOG if it keeps one.
- **No-loss check and self-review loop.** After applying, every removed line must be found at its new home or on the verified-DELETE list, and the final file re-read end-to-end for coherence — then an adversarial self-review pass hunts for loss, broken references, and introduced contradictions, fixing and re-reviewing until a pass comes back clean, one commit per iteration.
...
1. **Reversibility gate** — an instruction file is edited autonomously only when every change is individually revertible (git-tracked targets with per-iteration commits, or snapshot-protected memory); unversioned, unprotected files are never edited before the user approves their plan; user-only decisions — intent, preference, scope ownership, out-of-repo writes — always stop for the user, regardless of tier. (Rewrote the former stop-and-confirm invariant; explicit user approval, 2026-07-08.)
